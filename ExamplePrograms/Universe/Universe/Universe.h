...
namespace Universe {
	...
	*/
	...
	struct Vector2 {
		...
		friend Vector2 operator*(const Int left, const Vector2& right);
	...
	public:
		...
		Particle(Universe* universe, Vector2 position = Vector2(0, 0),
			const Int mass = 1, Vector2 velocity = Vector2(0, 0), const int visualRadius = 10);
	...
	private:
		...
		GUI::Window* space = nullptr;

		const long long gravityConstant = 2;
		const long long strongForceConstant = 0;
		const long long strongRadConstant = 10;


	public:

		Universe(const Vector2& universeSize = Vector2(500, 500), const double visualScale = 1,
		         const long long nParticles = 100, const int refreshTimeMs = 0);

		void Start();

		Int (* const Distance)(const Vector2& firstPoint, const Vector2& secondPoint) = ClassicDistance;

		GUI::Window* Space() const;

		Vector2 Size() const;

		double VisualDownscale() const;
	...
	private:
		...
		Vector2 Gravitation(Particle& first, Particle& second, Int distance);

		Vector2 StrongForce(Particle& first, Particle& second, Int distance);

		Vector2 size;
		double visualDownscale;
